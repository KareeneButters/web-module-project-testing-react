...
describe('Stranger Things App', () => {
  ...
  test('App renders the correct texts', async () => {
    ...
    await waitFor(() => {
      ...
      screen.getByText("A love letter to the '80s classics that captivated a generation"
         , { exact: false})
    ...
    screen.getByText("Chapter One: MADMAX")
          screen.getByText("One year after the events with the Upside Down and the Demogorgon"
       , { exact: false})
  })
})
